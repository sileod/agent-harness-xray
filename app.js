...
function renderOverviewMetrics() {
    ...
    Object.keys(AGENTS).forEach(key => {
        ...
        const dynamicTotal = (agent.overhead.skillsContext || 0) + (agent.overhead.meta || 0);
        if (dynamicTotal > maxDynamic) {
            maxDynamic = dynamicTotal;
            dynamicAgent = agent;
        }
    });
    ...
    const metrics = [
        {
            label: "Largest Capture",
            value: `${(largestTokens / 1000).toFixed(1)}k`,
            detail: "Rough provider-visible tokens",
        ...
        },
        {
            ...
            detail: "Named tools captured",
            agent: toolHeavyAgent.name,
            color: toolHeavyAgent.color
        },
        {
            label: "Dynamic/Meta",
            value: `${maxDynamic.toLocaleString()}`,
            detail: "Rules/session rough tokens",
            agent: dynamicAgent.name,
            color: dynamicAgent.color
        }
    ];
...
function initPromptViewer() {
    ...
    select.value = Object.keys(AGENTS)[0];
...
function updatePromptViewer() {
    ...
    statsContainer.innerHTML = `
        ...
        <div class="prompt-stat">Version: <strong>${agent.capturedVersion || 'unknown'}</strong></div>
    ...
    content.innerHTML = `
        <div class="prompt-section" style="--section-color: ${cat.color}">
            ...
            <p style="font-size: 0.85rem; color: var(--text-secondary); margin-bottom: var(--space-md)">
                Captured Rough Weight: <strong style="font-family: var(--font-mono); color: var(--text-primary)">${sec.tokens.toLocaleString()} tokens</strong> 
            ...
            <pre class="prompt-text"><code>${escapeHTML(getPromptSectionContent(sec))}</code></pre>
        </div>
    `;
}
...
function initTokenBudget() {
    ...
    budgetAgentSelect.value = Object.keys(AGENTS)[0];
...
function getPromptSectionContent(section) {
    if (section.contentKey && window.CAPTURED_PROMPTS && window.CAPTURED_PROMPTS[section.contentKey]) {
        return window.CAPTURED_PROMPTS[section.contentKey];
    }
